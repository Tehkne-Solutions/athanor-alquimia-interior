import { Archive, ArrowRight, BookOpenText, CheckCircle2, Circle, Clock3, FolderOpen, History, Inbox, Layers3, MoonStar, RefreshCw, ShieldCheck, Sparkles } from 'lucide-react';
...
export function ContinuousCyclePage() {
  ...
  return <div className="page page--continuous-cycle"><PageHeader eyebrow="O Ciclo que Retorna ao Templo" title="Ative uma jornada sem reiniciar o caminho." description="Cada instância referencia uma Nova Obra registrada, mantém seu próprio estado e pode ser pausada, encerrada ou arquivada sem afetar ciclos anteriores."/>
    <div className="continuous-cycle-summary">
      ...
      <Card title="Estado das jornadas" eyebrow={`${cycleProgress.instances.length} instâncias`}><ul className="simple-list"><li><strong>{summary.active}</strong> ativas</li><li><strong>{summary.paused}</strong> pausadas</li><li><strong>{summary.closed}</strong> encerradas</li><li><strong>{summary.archived}</strong> arquivadas</li></ul><div className="continuous-instance-actions"><Button variant="secondary" onClick={() => navigate('/temple/continuous-map')}>Abrir mapa dos ciclos</Button><Button variant="secondary" onClick={() => navigate('/temple/continuous-collections')}><FolderOpen size={17}/> Abrir coleções</Button><Button variant="secondary" onClick={() => navigate('/temple/continuous-received')}><Inbox size={17}/> Biblioteca recebida</Button></div></Card>
    </div>
    ...
    <Card title="Limites do ciclo contínuo" eyebrow="Sem progressão por repetição"><div className="safety-summary"><ShieldCheck/><p>Nenhum número de ativações, retomadas ou ciclos aumenta nível, restauração ou recompensa.</p></div><ul className="simple-list">{continuousCycleRestrictions.map((restriction) => <li key={restriction}>{restriction}</li>)}</ul><div className="continuous-instance-actions"><Button variant="secondary" onClick={() => navigate('/temple/continuous-map')}>Mapa dos ciclos</Button><Button variant="secondary" onClick={() => navigate('/temple/continuous-collections')}>Coleções</Button><Button variant="secondary" onClick={() => navigate('/temple/continuous-received')}><Inbox size={17}/> Biblioteca recebida</Button><Button variant="secondary" onClick={() => navigate('/temple/new-work')}>Registrar outra Nova Obra</Button><Button variant="ghost" onClick={() => navigate('/temple')}>Voltar ao Átrio</Button></div></Card>
  </div>;
}
