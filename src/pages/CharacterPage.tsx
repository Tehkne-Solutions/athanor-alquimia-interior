import { Award, Compass, Droplets, Settings2, Shield, Sparkles } from 'lucide-react';
...
export function CharacterPage() {
  ...
  return (
    <div className="page">
      <PageHeader eyebrow="Ficha RPG" title={character.name} description="Status de progressão do jogo, separados de qualquer avaliação clínica ou moral." action={<Button variant="ghost" onClick={() => navigate('/settings/accessibility')}><Settings2 size={18}/> Acessibilidade</Button>} />
      <div className="character-sheet">
        ...
        <Card title="Nível da Obra" eyebrow="Progressão">
          <div className="stat-block"><strong>{levelLabels[character.workLevel]}</strong><span>{level.index} de 6 faixas narrativas</span></div>
          <div className="progress-track"><span style={{width: level.width}}/></div>
        </Card>
        ...
        <Card title="Virtudes em prática" eyebrow="Experiências concluídas">
          <div className="virtue-list">
            <span><Sparkles/> Prudência <b>{inventory.length ? 2 : 0}</b></span>
            <span><Shield/> Clareza <b>{integrated ? 2 : inventory.length ? 1 : 0}</b></span>
            <span><Droplets/> Presença <b>{waterCompleted ? 4 : 0}</b></span>
          </div>
        ...
        <Card title="Ciclos revisados" eyebrow="Retorno à Obra">
          <div className="stat-block"><strong>{reviews.length + Number(waterCompleted)}</strong><span>{reviews.length + Number(waterCompleted) === 1 ? 'ciclo ou revisão registrada' : 'ciclos e revisões registrados'}</span></div>
          <p className="muted">Revisar, ajustar, repousar e arquivar contam como formas válidas de acompanhar uma jornada.</p>
        </Card>
      </div>
    </div>
  );
