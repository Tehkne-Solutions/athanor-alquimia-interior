import { BookOpenText, CheckCircle2, Footprints, ListChecks, LockKeyhole, ShieldCheck, Sprout } from 'lucide-react';
...
export function GardenPage() {
  ...
  return <div className="page page--earth"><PageHeader eyebrow="Jardim Interior" title={resourcesProgress?.status === 'completed' ? 'Os recursos possíveis receberam um recipiente.' : workProgress?.status === 'completed' ? 'Uma unidade pequena recebeu forma.' : bodyProgress?.status === 'completed' ? 'A presença corporal recebeu sua primeira marca.' : 'A Terra começa pelo que já está presente.'} description="O capítulo organiza corpo percebido, trabalho possível e recursos sem diagnóstico ou cobrança de produtividade."/><div className="earth-foundation-grid">
    <Card className="earth-hero-card"><div className="earth-hero-symbol" aria-hidden="true">{resourcesProgress?.status === 'completed' ? <ListChecks/> : workProgress?.status === 'completed' ? <Sprout/> : bodyProgress?.status === 'completed' ? <Footprints/> : <Sprout/>}</div><div><p className="eyebrow">Estado do Jardim</p><h2>{resourcesProgress?.status === 'completed' ? 'Terceiro componente criado' : workProgress?.status === 'completed' ? 'Segundo componente criado' : bodyProgress?.status === 'completed' ? 'Primeiro componente criado' : bodyProgress ? 'Missão em andamento' : 'Fundação disponível'}</h2><p>{resourcesProgress?.status === 'completed' ? 'O Cesto registra disponibilidade, substituição, escopo e uma decisão recusável.' : workProgress?.status === 'completed' ? 'A Semente registra uma unidade fictícia e uma decisão recusável.' : bodyProgress?.status === 'completed' ? 'A Marca registra escolhas curadas sem avaliar saúde, disciplina ou produtividade.' : 'A primeira missão trabalha somente com escolhas curadas, recusáveis e armazenadas localmente.'}</p></div></Card>
