import { BookOpenText, CheckCircle2, Clock3, Footprints, Gem, ListChecks, LockKeyhole, Map, ShieldCheck, Sparkles, Sprout } from 'lucide-react';
...
import { earthStoneFunctions } from '../content/earthStone';
...
export function GardenPage() {
  ...
  const rawStoneProgress = useEarthStoneStore((state) => state.progress);
  ...
  const sourceResourceBasketId = resourcesProgress?.status === 'completed' && resourcesProgress.possibleResourcesBasketCreated
    ? resourcesProgress.completedAt ?? `${resourcesProgress.sourceFirstStepSeedId}:resource-basket`
    : undefined;
  ...
  const sourceOrderMapId = orderProgress?.status === 'completed' && orderProgress.possibleOrderMapCreated
    ? orderProgress.completedAt ?? `${orderProgress.sourceRhythmCompassId}:order-map`
    : undefined;
  const stoneProgress = sourceOrderMapId && rawStoneProgress?.sourceOrderMapId === sourceOrderMapId ? rawStoneProgress : undefined;
  ...
  const selectedStoneFunction = earthStoneFunctions.find((option) => option.id === stoneProgress?.function);
  ...
  const stoneLabel = earthCompleted
    ? 'Revisar o ciclo da Terra'
    : stoneProgress?.positioned
      ? 'Concluir o capítulo da Terra'
      : stoneProgress?.status === 'integrated'
        ? 'Posicionar a Pedra'
        : stoneProgress?.stoneCreated
          ? 'Continuar o ciclo da Pedra'
          : stoneProgress
            ? 'Continuar a lapidação'
            : 'Lapidar a Pedra do Primeiro Passo';

  const gardenTitle = earthCompleted
    ? 'O Jardim foi restaurado e o Santuário está aberto.'
    : stoneProgress?.positioned
      ? 'A Pedra integrada ocupa seu lugar e aguarda a revisão geral.'
      : stoneProgress?.stoneCreated
        ? 'Os cinco componentes receberam uma Pedra em revisão.'
  ...
  return <div className="page page--earth"><PageHeader eyebrow={earthCompleted ? 'Jardim Interior · Restaurado' : 'Jardim Interior'} title={gardenTitle} description="O capítulo organiza corpo percebido, trabalho possível, recursos, ritmo e ordem sem diagnóstico ou cobrança de produtividade."/><div className="earth-foundation-grid">
    <Card className="earth-hero-card"><div className="earth-hero-symbol" aria-hidden="true">{earthCompleted ? <Sparkles/> : stoneProgress?.stoneCreated ? <Gem/> : orderProgress?.status === 'completed' ? <Map/> : rhythmProgress?.status === 'completed' ? <Clock3/> : resourcesProgress?.status === 'completed' ? <ListChecks/> : workProgress?.status === 'completed' ? <Sprout/> : bodyProgress?.status === 'completed' ? <Footprints/> : <Sprout/>}</div><div><p className="eyebrow">Estado do Jardim</p><h2>{earthCompleted ? 'Primeiro ciclo da Terra registrado' : stoneProgress?.positioned ? 'Pedra posicionada' : stoneProgress?.stoneCreated ? 'Item da Terra criado' : orderProgress?.status === 'completed' ? 'Cinco componentes disponíveis' : rhythmProgress?.status === 'completed' ? 'Quarto componente criado' : resourcesProgress?.status === 'completed' ? 'Terceiro componente criado' : workProgress?.status === 'completed' ? 'Segundo componente criado' : bodyProgress?.status === 'completed' ? 'Primeiro componente criado' : bodyProgress ? 'Missão em andamento' : 'Fundação disponível'}</h2><p>{earthCompleted ? 'A revisão geral preservou os destinos escolhidos e abriu o Santuário do Espírito.' : stoneProgress?.stoneCreated ? `A Pedra organiza ${selectedStoneFunction?.label.toLowerCase() ?? 'uma função local'} sem garantir execução ou resultado.` : orderProgress?.status === 'completed' ? 'A receita da Pedra do Primeiro Passo está disponível para lapidação e revisão.' : rhythmProgress?.status === 'completed' ? 'O Compasso registra uma cadência mínima, interrompível e sem sequência obrigatória.' : resourcesProgress?.status === 'completed' ? 'O Cesto registra disponibilidade, substituição, escopo e uma decisão recusável.' : workProgress?.status === 'completed' ? 'A Semente registra uma unidade fictícia e uma decisão recusável.' : bodyProgress?.status === 'completed' ? 'A Marca registra escolhas curadas sem avaliar saúde, disciplina ou produtividade.' : 'A primeira missão trabalha somente com escolhas curadas, recusáveis e armazenadas localmente.'}</p></div></Card>
    ...
    {bodyProgress?.status === 'completed' && <Card title="O Trabalho que Cabe Hoje" eyebrow="Segunda missão da Terra"><div className="earth-mission-preview">{workProgress?.status === 'completed' ? <CheckCircle2/> : <Sprout/>}<div><strong>{workProgress?.status === 'completed' ? 'Semente do Primeiro Passo criada' : 'Decomposição sem cobrança'}</strong><p>{workProgress?.status === 'completed' ? `${selectedStep?.label ?? 'Nenhum passo'} · ${selectedWorkDecision?.label ?? 'Nenhuma ação'}` : 'Intenção, projeto, tarefa, primeiro passo, capacidade e tempo percebidos.'}</p></div></div><Button onClick={() => navigate('/mission/work-that-fits-today')}>{workLabel}</Button></Card>}
    {workProgress?.status === 'completed' && <Card title="A Casa dos Recursos" eyebrow="Terceira missão da Terra"><div className="earth-mission-preview">{resourcesProgress?.status === 'completed' ? <CheckCircle2/> : <ListChecks/>}<div><strong>{resourcesProgress?.status === 'completed' ? 'Cesto dos Recursos Possíveis criado' : 'Inventário sem promessa'}</strong><p>{resourcesProgress?.status === 'completed' ? `${selectedResourceScope?.label ?? 'Escopo não definido'} · ${selectedResourceDecision?.label ?? 'Nenhuma ação'}` : 'Tempo, espaço, informação, materiais, apoio, substituição e redução de escopo.'}</p></div></div><Button onClick={() => navigate('/mission/house-of-resources')}>{resourcesLabel}</Button></Card>}
    {resourcesProgress?.status === 'completed' && <Card title="O Ritmo que Pode Ser Mantido" eyebrow="Quarta missão da Terra"><div className="earth-mission-preview">{rhythmProgress?.status === 'completed' ? <CheckCircle2/> : <Clock3/>}<div><strong>{rhythmProgress?.status === 'completed' ? 'Compasso do Ritmo Sustentável criado' : 'Cadência sem streak'}</strong><p>{rhythmProgress?.status === 'completed' ? `${selectedRhythmFrequency?.label ?? 'Nenhuma frequência'} · ${selectedRhythmDecision?.label ?? 'Nenhuma ação'}` : 'Ritmo, pressa, repetição, cobrança, pausa, recursos e retomada.'}</p></div></div><Button onClick={() => navigate('/mission/sustainable-rhythm')}>{rhythmLabel}</Button></Card>}
    {rhythmProgress?.status === 'completed' && <Card title="A Ordem que Serve" eyebrow="Quinta missão da Terra"><div className="earth-mission-preview">{orderProgress?.status === 'completed' ? <CheckCircle2/> : <Map/>}<div><strong>{orderProgress?.status === 'completed' ? 'Mapa da Ordem Possível criado' : 'Prioridade sem urgência'}</strong><p>{orderProgress?.status === 'completed' ? `${selectedOrderPriority?.label ?? 'Nenhuma prioridade'} · ${selectedOrderDecision?.label ?? 'Nenhuma ação'}` : 'Ordem, prioridade, rigidez, acúmulo, limite visível e revisão.'}</p></div></div><Button onClick={() => navigate('/mission/order-that-serves')}>{orderLabel}</Button></Card>}
    {orderProgress?.status === 'completed' && <Card title="Pedra do Primeiro Passo" eyebrow="Crafting da Terra"><div className="earth-mission-preview">{earthCompleted ? <Sparkles/> : stoneProgress?.stoneCreated ? <Gem/> : <CheckCircle2/>}<div><strong>{earthCompleted ? 'Ciclo da Terra concluído' : stoneProgress?.positioned ? 'Pedra pronta para revisão geral' : stoneProgress?.stoneCreated ? 'Pedra em ciclo de revisão' : 'Cinco componentes reunidos'}</strong><p>{earthCompleted ? 'O Jardim está restaurado e o Santuário do Espírito está disponível.' : stoneProgress?.positioned ? 'A integração e o posicionamento não encerram o capítulo sem revisão geral.' : stoneProgress?.stoneCreated ? 'Criada sem integração automática.' : 'Função, passo, recurso, ritmo, limite e revisão.'}</p></div></div><Button onClick={() => navigate(earthCompleted || stoneProgress?.positioned ? '/review/earth-chapter' : '/crafting/first-step-stone')}>{stoneLabel}</Button></Card>}
    ...
    {stoneProgress?.stoneCreated && <Card title="Pedra do Primeiro Passo" eyebrow="Item da Terra"><div className="earth-mission-preview"><Gem/><div><strong>{earthCompleted ? 'Ciclo registrado' : stoneProgress.positioned ? 'Posicionada no Jardim' : stoneProgress.status === 'integrated' ? 'Integrada' : stoneProgress.status === 'resting' ? 'Em repouso' : 'Ciclo em revisão'}</strong><p>Não representa estabilidade, produtividade, cura ou direção garantida.</p></div></div></Card>}
    {earthCompleted && <Card title="Santuário do Espírito" eyebrow="Próximo capítulo disponível"><div className="earth-mission-preview"><Sparkles/><div><strong>Fundação do quinto elemento aberta</strong><p>Palavra, emoção, impulso, corpo e ação serão integrados sem leitura oculta ou exigência de completude.</p></div></div><Button onClick={() => navigate('/temple/spirit-sanctuary')}>Entrar no Santuário</Button></Card>}
