import { BookOpenText, CheckCircle2, Clock3, Footprints, Gem, ListChecks, LockKeyhole, Map, ShieldCheck, Sprout } from 'lucide-react';
...
export function GardenPage() {
  ...
  const sourceOrderMapId = orderProgress?.status === 'completed' && orderProgress.possibleOrderMapCreated ? orderProgress.completedAt ?? `${orderProgress.sourceRhythmCompassId}:order-map` : undefined;
  ...
  const stoneLabel = stoneProgress?.positioned ? 'Revisar a Pedra posicionada' : stoneProgress?.status === 'integrated' ? 'Posicionar a Pedra' : stoneProgress?.stoneCreated ? 'Continuar o ciclo da Pedra' : stoneProgress ? 'Continuar a lapidação' : 'Lapidar a Pedra do Primeiro Passo';

  const gardenTitle = stoneProgress?.positioned
    ? 'A Pedra integrada ocupa seu lugar no Jardim.'
  ...
  return <div className="page page--earth"><PageHeader eyebrow="Jardim Interior" title={gardenTitle} description="O capítulo organiza corpo percebido, trabalho possível, recursos, ritmo e ordem sem diagnóstico ou cobrança de produtividade."/><div className="earth-foundation-grid">
    ...
    {orderProgress?.status === 'completed' && <Card title="Pedra do Primeiro Passo" eyebrow="Crafting da Terra"><div className="earth-mission-preview">{stoneProgress?.stoneCreated ? <Gem/> : <CheckCircle2/>}<div><strong>{stoneProgress?.stoneCreated ? 'Pedra em ciclo de revisão' : 'Cinco componentes reunidos'}</strong><p>{stoneProgress?.positioned ? 'Integrada e posicionada no Jardim.' : stoneProgress?.stoneCreated ? 'Criada sem integração automática.' : 'Função, passo, recurso, ritmo, limite e revisão.'}</p></div></div><Button onClick={() => navigate('/crafting/first-step-stone')}>{stoneLabel}</Button></Card>}
