import { ArrowRight, BadgeCheck, BookOpenText, CheckCircle2, Clock3, CupSoda, Database, Droplets, Flame, Hammer, LampDesk, RefreshCw, Shield, ShieldCheck } from 'lucide-react';
...
export function TemplePage() {
  ...
  const roomSelect = (roomId: string) => { if (roomId === 'proverbs-library') navigate('/temple/proverbs-library'); if (roomId === 'psalms-chamber') navigate('/temple/psalms-chamber'); if (roomId === 'forge') navigate('/temple/forge'); };
  ...
  const fireAction = transformedMetalCreated ? 'Revisar o Metal Transformado' : transformationProgress ? 'Continuar O que Precisa Ser Transformado' : courageMarkCreated ? 'Iniciar O que Precisa Ser Transformado' : courageProgress ? 'Continuar A Coragem Proporcional' : boundaryPlateCreated ? 'Iniciar A Coragem Proporcional' : boundaryProgress ? 'Continuar O Limite que Protege' : intervalEmberCreated ? 'Iniciar O Limite que Protege' : intervalProgress ? 'Continuar O Instante Antes do Gesto' : namedFlameCreated ? 'Iniciar O Instante Antes do Gesto' : fireProgress ? 'Continuar O Nome da Chama' : 'Iniciar O Nome da Chama';
  const fireRoute = courageMarkCreated ? '/mission/what-needs-transformation' : boundaryPlateCreated ? '/mission/proportional-courage' : intervalEmberCreated ? '/mission/limit-that-protects' : namedFlameCreated ? '/mission/before-the-gesture' : '/mission/name-the-flame';
  const activeRoomCount = temple.rooms.filter((room) => ['active', 'restored', 'available'].includes(room.status)).length;
  const componentCount = inventory.length + Number(waterJourney?.namedDropCreated) + Number(chaliceAvailable) + Number(namedFlameCreated) + Number(intervalEmberCreated) + Number(boundaryPlateCreated) + Number(courageMarkCreated) + Number(transformedMetalCreated);
  ...
  return <div className="page page--temple"><PageHeader eyebrow="Átrio da Presença" title={`Bem-vindo ao Templo, ${character.name}.`} description="Seu Templo registra ciclos de gameplay, itens e revisões. Ele não mede sua condição espiritual." action={<span className="local-badge"><Database size={16}/> Dados locais</span>}/><div className="temple-dashboard">
    <Card className="hero-card"><div className="hero-card__scene"><div className={`temple-backdrop temple-backdrop--${temple.theme}`} aria-hidden="true"/><CharacterAvatar character={character}/></div><div className="hero-card__content"><p className="eyebrow">Nível da Obra</p><h2>{workLevelLabels[character.workLevel]}</h2><p>{transformedMetalCreated ? 'A quinta prática do Fogo registrou uma transformação fictícia sem concluir automaticamente o capítulo.' : courageMarkCreated ? 'A quarta prática do Fogo registrou uma ação proporcional sem concluir automaticamente o capítulo.' : boundaryPlateCreated ? 'A terceira prática do Fogo registrou um limite em primeira pessoa.' : intervalEmberCreated ? 'A segunda prática do Fogo registrou um intervalo seguro.' : namedFlameCreated ? 'A primeira prática do Fogo foi concluída.' : waterCompleted ? 'O ciclo da Água foi registrado e a Forja foi aberta.' : integrated ? 'A primeira Obra foi revisada.' : 'A Biblioteca aguarda sua primeira restauração.'}</p><div className="status-row"><span><strong>{activeRoomCount}</strong> salas acessíveis</span><span><strong>{componentCount}</strong> componentes e itens</span><span><strong>{reviews.length + Number(waterCompleted)}</strong> ciclos e revisões</span><span><strong>{temple.restorationLevel}</strong> nível do Templo</span></div></div></Card>
    ...
    {forgeAvailable && <Card eyebrow={transformedMetalCreated ? 'Quinto componente criado' : courageMarkCreated ? 'Quarto componente criado' : boundaryPlateCreated ? 'Terceiro componente criado' : intervalEmberCreated ? 'Segundo componente criado' : namedFlameCreated ? 'Primeiro componente criado' : 'Novo capítulo disponível'} title="A Forja dos Elementos" className="mission-card mission-card--fire"><div className="mission-card__icon">{transformedMetalCreated ? <Hammer/> : courageMarkCreated ? <BadgeCheck/> : boundaryPlateCreated ? <Shield/> : intervalEmberCreated ? <Clock3/> : <Flame/>}</div><p>{transformedMetalCreated ? 'O Metal registra destino, intervenção fictícia, salvaguarda e revisão.' : courageMarkCreated ? 'A Marca está pronta para uma prática de transformação segura.' : 'O Fogo avança por componentes separados e revisáveis.'}</p><Button variant="secondary" onClick={() => navigate(fireRoute)}>{fireAction} <ArrowRight size={18}/></Button></Card>}
