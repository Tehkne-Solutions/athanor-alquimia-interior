...
export function TemplePage() {
  ...
  return <div className="page page--temple"><PageHeader eyebrow="Átrio da Presença" title={`Bem-vindo ao Templo, ${character.name}.`} description="Seu Templo registra ciclos de gameplay, itens e revisões. Ele não mede sua condição espiritual." action={<span className="local-badge"><Database size={16}/> Dados locais</span>}/><div className="temple-dashboard">
    ...
    {forgeAvailable && <Card eyebrow={shieldProgress?.positioned ? 'Item posicionado' : shieldCreated ? 'Escudo criado' : transformedMetalCreated ? 'Cinco componentes reunidos' : courageMarkCreated ? 'Quarto componente criado' : boundaryPlateCreated ? 'Terceiro componente criado' : intervalEmberCreated ? 'Segundo componente criado' : namedFlameCreated ? 'Primeiro componente criado' : 'Novo capítulo disponível'} title="A Forja dos Elementos" className="mission-card mission-card--fire"><div className="mission-card__icon">{shieldCreated ? <Shield/> : transformedMetalCreated ? <Hammer/> : courageMarkCreated ? <BadgeCheck/> : boundaryPlateCreated ? <Shield/> : intervalEmberCreated ? <Clock3/> : <Flame/>}</div><p>{shieldProgress?.positioned ? 'O Escudo está posicionado, mas o capítulo ainda terá encerramento próprio.' : shieldCreated ? 'O Escudo possui ciclo explícito de revisão e integração.' : transformedMetalCreated ? 'Os cinco componentes podem formar uma estrutura revisável.' : 'O Fogo avança por componentes separados e recusáveis.'}</p><Button variant="secondary" onClick={() => navigate(fireRoute)}>{fireAction} <ArrowRight size={18}/></Button></Card>}
