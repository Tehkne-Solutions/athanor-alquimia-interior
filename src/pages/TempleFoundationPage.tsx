...
export function TempleFoundationPage() {
  ...
  return (
    <main className="onboarding-page" id="main-content">
      ...
      <header className="onboarding-header"><p className="eyebrow">Fundação · ambiente</p><h1>Escolha a primeira forma do seu Templo.</h1><p>A escolha é estética, reversível e não modifica fontes, recompensas ou resultados da jornada.</p></header>
      <div className="temple-theme-grid" role="group" aria-label="Temas visuais do Templo">
        {themes.map(({ id, name, icon: Icon, description }) => <button key={id} type="button" aria-pressed={selected === id} onClick={() => setSelected(id)} className={`temple-theme temple-theme--${id} ${selected === id ? 'temple-theme--selected' : ''}`}><div className="temple-theme__scene"><Icon size={42}/><span className="temple-theme__light"/></div><strong>{name}</strong><span>{description}</span></button>)}
      </div>
