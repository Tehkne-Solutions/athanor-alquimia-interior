...
const allSpiritNodes = [...spiritFoundationNodes, ...spiritThreadNodes, ...spiritCenterNodes, ...spiritCouncilNodes];
...
export function SpiritSanctuaryPage() {
  ...
  const sourceCenterId = centerCompleted && centerProgress
    ? centerProgress.completedAt ?? `${centerProgress.sourceThreadId}:provisional-center-knot`
    : undefined;
  ...
  const heroState = councilCompleted
    ? { title: 'Terceiro componente criado', description: 'O Selo preserva fala, passagem, desconhecimento e discordância sem estabelecer maioria como verdade.' }
    : councilProgress
